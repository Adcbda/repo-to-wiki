...
Use this skill to turn a local repository into a source-grounded engineering wiki. The workflow connects catalog prompts to content prompts through one contract: catalog `source_hints` become retrieval seeds, then content generation turns them into verified evidence packs and cited pages.
...
3. Generate and validate the catalog.
   ...
   - Validate that every page has `title`, `type`, `purpose`, `answers`, `source_hints`, and `children`.
...
4. Build the content generation manifest.
   ...
   - Output: parent-before-child page manifest with paths, intent, evidence strategy, content shape, diagram candidates, and review focus.
...
5. Every page object contains `title`, `type`, `purpose`, `answers`, `source_hints`, and `children`.
6. `children` is always present; use `[]` for leaf pages.
7. All `source_hints` are relative to the analyzed repository root.
8. `metadata/wiki-structure.json` is created only from validated JSON. Invalid raw model output belongs in `metadata/wiki-structure.raw.md`.
...
2. `evidence_strategy` in the generation manifest describes retrieval intent.
3. `source-evidence-selection-prompt.md` produces the only valid citation namespace for the page.
4. Save that output to `metadata/evidence/<page-slug>.json` before drafting the page.
5. Page Markdown may cite only IDs present in its evidence JSON.
6. Page Markdown must render citations as clickable links in `Sources:` lines:
